...
int run_gui(const Config& config) {
  ...
  auto create_window = [&]() {
    set_gl_hints();
    return glfwCreateWindow(800, 480, "Audio Recorder " RECORDER_VERSION, nullptr, nullptr);
  };
#else
  ...
  auto create_window = [&]() {
    ...
    glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);
    glfwWindowHint(GLFW_DECORATED, GLFW_FALSE);  // irrilevante in fullscreen esclusivo, ma innocuo
    GLFWmonitor* primary = glfwGetPrimaryMonitor();
    const GLFWvidmode* mode = primary ? glfwGetVideoMode(primary) : nullptr;
    if (mode) {
      // Fullscreen esclusivo: passiamo `primary` come monitor invece di
      // nullptr. In precedenza si era scelta la modalita' finestra
      // apposta per evitare l'esclusivo (rischio di schermo nero dopo
      // un'iconificazione su alcuni window manager) — ripristinato ora
      // su richiesta, avendo uno schermo dedicato interamente all'app
      // (nessun desktop condiviso da gestire). glfwIconifyWindow (usato
      // dal bottone Desktop) dovrebbe comunque continuare a funzionare:
      // GLFW ripristina il fullscreen alla de-iconificazione. Se sullo
      // schermo reale questo dovesse dare problemi, tornare a
      // nullptr qui sotto ripristina la modalita' finestra precedente.
  ...
  if (!window) {
   ...
   // Su Raspberry Pi / alcuni WM Linux una finestra borderless a tutto
  // schermo puo' partire minimizzata o fuori viewport se non viene
  // posizionata e portata in primo piano esplicitamente.
